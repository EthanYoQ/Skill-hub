...
description: |
  Produce comprehensive synthesis with explicit citations from multiple ordinary public-web sources via the Tavily CLI. Use for detailed reports, comparisons, market or literature research, and questions whose answer requires multi-source web evidence rather than a quick lookup. Do not use for simple discovery (tavily-search), one known URL (tavily-extract), one-site URL discovery or bulk extraction (tavily-map/crawl), or research centered on named social, community, career, code, media, authenticated, or cross-platform sources (Agent Reach). Takes 30-120 seconds.
