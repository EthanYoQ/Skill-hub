...
def _strip_reader_visible_mermaid(html: str) -> str:
    """Remove Mermaid source blocks from reader-facing subpage HTML.

    Rendered PNG assets and the Mermaid render manifest carry the visual and
    source contract. Persisted reader pages should not expose raw flowchart text.
    """
    html = re.sub(
...
def persist_sub_pages(
    ...
    for entry in sub_pages:
        slug = entry["slug"]
        html_body = _strip_reader_visible_mermaid(str(entry["html"]))
