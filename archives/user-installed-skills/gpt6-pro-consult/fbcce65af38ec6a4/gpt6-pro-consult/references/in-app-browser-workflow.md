...
3. 使用当前工具声明的 in-app Browser 入口。CUA 的入口调用会返回 documentation 和初始页面状态，阅读后使用其 API；不要再寻找旧版 `agent.browsers`、`tab.playwright` 或额外的 documentation 方法。
...
只通过当前 task 的 in-app Browser 和当前 documentation 所列的页面 API 操作该页面。不要改用 Chrome 扩展、外部 Playwright、OpenCLI 或浏览器 CLI。
...
仅在当前 documentation 提供 session 命名能力时使用；命名不是发送门槛，也不是绑定证据。新 task 不得沿用旧 task 的 tab 变量或 conversation ID。
...
4. 缺少任一信号时修复草稿；不要在未验证内容时提交。核对通过并记录发送前证据后，直接继续第 5 节，不停在“已输入”的进度说明，也不再重写材料。
...
在同一 tab 中观察生成状态。存在 stop control、thinking 状态或未完成 assistant turn 时继续等待，不刷新、不发送“继续”。区分材料准备、已输入未提交、已提交生成中；数分钟生成本身不是发送失败，进度说明应报告观测到的阶段。
