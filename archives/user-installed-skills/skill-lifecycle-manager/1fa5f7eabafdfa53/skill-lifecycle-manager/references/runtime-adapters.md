...
| --- | --- | --- |
| Codex user | `~/.agents/skills` | User or source repository; Codex loads this root globally |
| Codex user compatibility | `$CODEX_HOME/skills`, normally `~/.codex/skills` | Deprecated compatibility root; migrate user-owned skills and preserve managed `.system` content |
| Codex project | `.agents/skills` from the working directory through the repository root | Project repository |
| Codex project compatibility | project `.codex/skills` | Legacy project state; prefer the current `.agents/skills` location |
| Shared Agent Skills root | another host-neutral Agent Skills root | Runtime remains `agent-skills` or `unknown` until loader evidence identifies a host |
...
For current Codex installs, place new user-owned skills in `~/.agents/skills`.
When migrating from `$CODEX_HOME/skills`, move only ordinary user skill
directories. Keep `$CODEX_HOME/skills/.system` and plugin caches under Codex
management, then validate discovery from a fresh Codex process before removing
any remaining compatibility copy.
...
Treat `~/.agents/skills` as a shared Agent Skills root that current Codex loads
at user scope. Confirm the actual consumer before labeling an entry as
exclusively Codex-owned. Report `runtime=agent-skills` or `runtime=unknown`
until loader evidence establishes Codex or another host; once established,
record that evidence without treating the directory name as ownership proof.
