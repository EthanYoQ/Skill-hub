...
| --- | --- | --- |
| `mirror` | Local source should match verified upstream | Replace only after normalized diff; direct replacement is the default, and `-WithBackup` is opt-in |
...
For current Codex installs, treat upstream fields such as
`disable-model-invocation`, `argument-hint`, and `user-invocable` as legacy
source metadata, not fields to mirror into `SKILL.md`. Preserve the required
`name` and `description`, represent explicit-only behavior with
`policy.allow_implicit_invocation: false` in `agents/openai.yaml`, and record
the installed copy as `mapped` so a later raw mirror cannot restore the legacy
fields.
...
5. Apply only a verified `mirror` drift. The default apply path directly
   replaces the target after containment checks; pass `-WithBackup` only when
   the user explicitly requests a backup. Review `mapped` and `manual`
   differences; never auto-apply `candidate`, `unknown`, `self`, or `local`
   entries.
...
- Clears the target directory before copying a verified mirror; it does not
  create a backup unless `-WithBackup` is passed.
- Returns `backupPath: null` for the default direct replacement and a path only
  when `-WithBackup` was explicitly used.
- Writes source-index status only in apply mode.

Report `backupPath: null` for the default direct replacement. When
`-WithBackup` is explicitly used, require a non-empty reported `backupPath` for
an applied mirror.

`BackupRoot` is used only with `-WithBackup`; if omitted in that mode, the
default is `<RepoRoot>/.backups/skill-upstream-audit/`.
...
- Mirrors a standalone copy only with `-Apply`, verified `mirror` policy, and
  path containment. It directly replaces the target by default; add
  `-WithBackup` for a timestamped backup.
...
Before local apply, review every discovered Git root. Source checkouts nested
under skill roots can appear in inventory, so exclude or interpret them
deliberately.
