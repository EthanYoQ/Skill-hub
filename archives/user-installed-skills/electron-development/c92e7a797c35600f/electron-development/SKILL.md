...
- Implementing Electron packaging configuration and validating packages locally with electron-builder or electron-forge
- Building and qualifying Electron macOS DMG/ZIP installers on GitHub-hosted macOS runners, including mounted packaged smoke and signing/notarization disclosure
- Implementing auto-update with electron-updater
- Implementing or debugging Electron packaging configuration and packaged smoke tests
...
- Building or qualifying Windows installers through GitHub Actions → use `github-desktop-release`
- Promoting artifacts, publishing/resuming a GitHub Release, or performing authoritative Release readback → use `github-desktop-release`
- Building a non-Electron macOS desktop installer → use the relevant platform/build Skill
...
- For a combined Windows + macOS release, invoke both Skills once: this Skill owns only macOS build/qualification; `github-desktop-release` owns Windows build/qualification plus shared promotion and authoritative readback.
...
- `github-desktop-release` — For Windows GitHub Actions qualification and for promotion, publication, resume, or authoritative readback of already-qualified desktop artifacts
