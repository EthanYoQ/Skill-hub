...
# 句末标点(中英文) + HTML块边界:用于划分句子/表格行。
#
# The verifier receives raw report HTML. Without table-row boundaries, a drug
# in one appendix row can be incorrectly matched to the citation in the
# previous row. Keep <td> out of the boundary set because evidence anchors often
# live in a sibling evidence cell within the same row.
_BOUNDARY_RE = re.compile(r"[。!?!?\n]|</(?:p|tr|li|section|div)>|<br\s*/?>", re.I)
...
def _sentence_span(text: str, position: int) -> tuple[int, int]:
    ...
    right = len(text)
    match = _BOUNDARY_RE.search(text, position)
    if match:
        right = match.end()
