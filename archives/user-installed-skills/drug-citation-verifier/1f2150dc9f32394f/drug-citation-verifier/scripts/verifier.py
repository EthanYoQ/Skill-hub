...
_A11_CANDIDATES = [
    Path(__file__).resolve().parents[2] / "citation-anchor-resolver" / "scripts",
    Path.home() / ".codex" / "skills" / "citation-anchor-resolver" / "scripts",
    Path.home() / ".claude" / "skills" / "citation-anchor-resolver" / "scripts",
]
_A11_SCRIPTS = next((path for path in _A11_CANDIDATES if path.exists()), None)
if _A11_SCRIPTS is not None:
    while str(_A11_SCRIPTS) in sys.path:
        sys.path.remove(str(_A11_SCRIPTS))
    ...
    for _module_name in ("resolver", "_source_loader", "_anchor_schema", "_keyword_match"):
        ...
        if _loaded_file and _A11_SCRIPTS not in _loaded_file.resolve().parents:
