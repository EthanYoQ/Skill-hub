...
_UPPER_HYPHEN_ABBREV_RE = re.compile(r"\b([A-Z]{2,}(?:-[A-Z0-9]{2,})+)\b")
_TRAILING_FORMULATION_RE = re.compile(r"[A-Z0-9]{1,3}")
_NON_DRUG_ABBREV_SUFFIXES: frozenset[str] = frozenset({"GM", "BDG", "NGS", "PCR", "CT"})
...
_CN_BOUNDARY_CHARS: frozenset[str] = frozenset(
    # 动作/状态动词
    "推荐治疗使用应用采用建议批准获批准予允许包括含有具有给予服用口服注射需讨论谈说述记载记录"
    # 关系/介词/连词
...
def _extend_trailing_formulation_suffix(text: str, end_pos: int) -> int:
    """Include short ASCII formulation/class suffixes after Chinese INN stems."""
    match = _TRAILING_FORMULATION_RE.match(text[end_pos : end_pos + 3])
    if not match:
        return end_pos
    return end_pos + len(match.group(0))
...
def _looks_like_treatment_abbrev(text: str, start: int, end: int) -> bool:
    name = text[start:end]
    last_segment = name.rsplit("-", 1)[-1]
    if last_segment in _NON_DRUG_ABBREV_SUFFIXES:
        return False
    context = _context_around(text, start, end)
    return any(keyword in context for keyword in DRUG_CONTEXT_KEYWORDS)
...
def extract_drug_mentions_with_context(text: str) -> list[dict]:
    ...
    for m in _CN_SUFFIX_RE.finditer(text):
        ...
        if span is None:
            continue
        start, end = span
        end = _extend_trailing_formulation_suffix(text, end)
    ...
    for m in _UPPER_HYPHEN_ABBREV_RE.finditer(text):
        start, end = m.start(), m.end()
        if (start > 0 and text[start - 1] in ":/") or (end < len(text) and text[end:end + 1] == ":"):
            continue
        if not _looks_like_treatment_abbrev(text, start, end):
            continue
        if start in seen_positions:
            continue
        seen_positions.add(start)
