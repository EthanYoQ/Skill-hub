...
## Project Override: Cover Is One Visual, Not the Whole System

For this project, use this skill only for article covers or true hero visuals. Do not use it to generate every body illustration.

Before creating a cover prompt, use `wechat-visual-director` to choose the cover route. The route must express the article's central tension, promise, or metaphor; it should not simply decorate the title.

Codex native image generation is preferred. If the native `image_gen`/`imagegen` tool is available, use it directly with the saved prompt content and requested aspect ratio. Do not route through another computer's Codex CLI unless explicitly requested.

Create at least two distinct cover routes when the article is important or when the user is unhappy with repetitive imagery. Distinct means different subject/metaphor/composition, not just another palette.
