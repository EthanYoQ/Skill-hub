...
description: 当用户询问 ClinicalTrials.gov / 临床试验 / NCT / 试验注册 / 招募状态 / phase 1/2/3 / RCT lookup / sponsor 检索 / interventional trial / observational study / 主次要终点 / trial outcomes / 试验招募中 / recruiting status 时,通过 ClinicalTrials.gov API v2 实时检索官方注册数据。专注实时单次查询(≤1000 条/请求),典型场景: "某 NCT 试验最新状态"、"BTK 抑制剂 Phase 3 在招募试验"、"某试验已发布的主次要终点结果"。明确边界: 批量 >10k 试验 / 全库 SQL 历史分析 → aact-bulk-trials; 关联 PubMed 文献 → pubmed-eutils; RCT 自动 GRADE 评级 → medical-evidence-grading; 引用入报告附录 → evidence-appendix-sync。跨平台无锁(Claude Code / Codex / Cursor 通用)。
keywords:
  - clinical-trials
  - clinicaltrials-gov
  - clinicaltrials.gov
