...
如果调用方提供了源 deck，可在该文件顶部 `<style>` 内按类名查找完整 CSS；没有源文件时，以本参考、`references/visual-grammar.md` 和 `assets/deck-shell.html` 为准。`assets/starter-template.html` 只是 legacy Pfizer component gallery，不是默认构图来源。
