...
Warm editorial 的稳定规则是 **display + body + data** 三个角色，不是固定三种字体。具体字族由 `style-index.json` 的选定 profile 决定；本文件后续 Fraunces / Inter / JetBrains Mono 细节属于 `warm-paper-terracotta` legacy profile。

所有 profile 必须满足：display 用于标题、引言和少量决定性数字；body 用于长文与标签；data 用于数值、代码、引用 ID 和 metadata。不得让 display 字体承担长篇正文。加载 web font 或嵌入 PPTX 字体前确认许可与可用性，并提供系统 fallback。

## Legacy Pfizer profile 三字族分工
...
HTML 横向全屏 deck 的默认目标是投影演讲，字号必须接近 `guizang-ppt-skill` 的屏幕尺度，同时使用选定 profile 的 display / body / data 字体角色。
...
❌ 在 legacy profile 中任意替换 Fraunces；其他 profile 使用索引中明确声明的 display 字体
