...
## Project Override: WeChat Visual Director First

For this project, do not let the Type × Style × Palette preset system become the creative ceiling.

Before Step 2 or any prompt writing, use the project-level `wechat-visual-director` skill to decide:

- Whether each visual should be a real screenshot, generated image, chart, or diagram.
- The role of each image: hook, proof, explanation, comparison, sequence, or summary.
- Two or three genuinely different routes for important visuals, differing in metaphor, composition, and visual language.

Only after that, use this skill to save outline/prompt files and generate the selected images.

Codex native image generation is preferred in this project. If the native `image_gen`/`imagegen` tool is available, call it directly with the saved prompt content and target aspect ratio. Do not route through another machine's Codex CLI unless the user explicitly asks for that.

Avoid these project-specific failure modes:

- Every image uses the same warm hand-drawn or cover-template look.
- Body images are decorative but do not clarify a claim.
- Generated screenshots or fake UI replace real product/repo screenshots.
- Chinese labels are dense, misspelled, or hard to read.
- Variations differ only by palette while the composition stays the same.
